...
describe("ServerModule API", () => {
  ...
  it("rejects protected route with wrong bearer", async () => {
    const res = await fetch("http://localhost:27125/api/status", { headers: { Authorization: "Bearer WRONG" } });
    expect(res.status).toBe(401);
  });

  it("allows protected route with correct bearer", async () => {
    const res = await fetch("http://localhost:27125/api/status", { headers: { Authorization: `Bearer ${key}` } });
    expect(res.status).toBe(200);
    const body = await res.json();
