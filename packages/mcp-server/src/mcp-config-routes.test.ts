...
describe("McpServer config routes", () => {
  ...
  it("rejects /config/key without auth", async () => {
    const res = await post("http://localhost:27126/config/key", { rotate: true });
  ...
  it("sets explicit newKey and allows auth with it", async () => {
    ...
    const shouldFail = await post("http://localhost:27126/config/key", { newKey }, key);
    ...
    const setExplicit = await post("http://localhost:27126/config/key", { newKey }, key);
    expect(setExplicit.status).toBe(200);

    // Now use new key for an authenticated but invalid (no Accept) call to confirm 4xx not 401.
    const check = await fetch(`http://localhost:27126/mcp?key=${encodeURIComponent(newKey)}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "x", version: "0" } } }) });
  ...
  it("rejects /config/storage without auth", async () => {
    const res = await post("http://localhost:27126/config/storage", { mode: "memory" });
  ...
  it("acknowledges storage mode change with auth", async () => {
    ...
    const res = await post("http://localhost:27126/config/storage", { mode: "memory" }, "explicit_new_key_456");
  ...
  it("returns 400 for invalid storage payload", async () => {
    const res = await post("http://localhost:27126/config/storage", { invalid: true }, "explicit_new_key_456");
