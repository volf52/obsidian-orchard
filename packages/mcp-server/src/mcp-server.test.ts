...
describe('McpServer (MCP SDK HTTP Transport)', () => {
  ...
  it('creates, lists, and updates tasks', async () => {
    ...
    expect(createdData.task.links.base?.path).toContain(
      'orchard-tasks.base.json',
    )
    ...
    const transitionedData = extractJsonContent(
      transitioned.body.result,
    ) as TaskResponse
    ...
    const filteredData = extractJsonContent(
      filtered.body.result,
    ) as ListTasksResponse
    expect(
      filteredData.tasks.some((t) => t.id === transitionedData.task.id),
    ).toBe(true)
    ...
    expect(extractErrorCode(rejectOutside.body.result)).toBe(
      'TaskOutsideFolder',
    )
  })
